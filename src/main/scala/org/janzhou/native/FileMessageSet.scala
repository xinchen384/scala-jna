...
private class FileMessageSet(
 ...
 // override 
  ...
  def iterator: Iterator[MessageAndOffset]  = { iterator(Int.MaxValue) }
    ...
    new IteratorTemplate[MessageAndOffset] {
      ...
      override def makeNext(): MessageAndOffset = {
        ...
        if(size > maxMessageSize)
          println( "CorruptRecordException" );
  ...
  //def append(messages: ByteBufferMessageSet) {
    ...
    val nsizeInBytes = buffer.limit
    buffer.mark()
    var written = 0
    while(written < nsizeInBytes)
  ...
  def truncateTo(targetSize: Int): Int = {
    ...
    if(targetSize > originalSize || targetSize < 0)
       println( "Attempt to truncate log segment to Kafka Exception!!!" )
